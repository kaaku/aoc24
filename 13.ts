import { areEqual, Point } from "./utils";

const INPUT = `\
Button A: X+62, Y+48
Button B: X+21, Y+74
Prize: X=3946, Y=6404

Button A: X+14, Y+66
Button B: X+79, Y+27
Prize: X=9393, Y=7469

Button A: X+15, Y+42
Button B: X+62, Y+29
Prize: X=367, Y=280

Button A: X+50, Y+92
Button B: X+99, Y+56
Prize: X=3890, Y=5896

Button A: X+61, Y+35
Button B: X+23, Y+77
Prize: X=3618, Y=3990

Button A: X+58, Y+11
Button B: X+32, Y+81
Prize: X=14400, Y=1768

Button A: X+26, Y+69
Button B: X+49, Y+35
Prize: X=508, Y=968

Button A: X+11, Y+21
Button B: X+54, Y+20
Prize: X=1996, Y=6718

Button A: X+47, Y+35
Button B: X+19, Y+54
Prize: X=1314, Y=2453

Button A: X+13, Y+44
Button B: X+73, Y+21
Prize: X=7316, Y=17026

Button A: X+20, Y+47
Button B: X+50, Y+27
Prize: X=3360, Y=6107

Button A: X+19, Y+65
Button B: X+39, Y+12
Prize: X=17960, Y=8711

Button A: X+24, Y+49
Button B: X+63, Y+30
Prize: X=7589, Y=18325

Button A: X+18, Y+53
Button B: X+94, Y+53
Prize: X=1532, Y=1378

Button A: X+19, Y+47
Button B: X+62, Y+17
Prize: X=5722, Y=15472

Button A: X+89, Y+68
Button B: X+11, Y+51
Prize: X=3176, Y=4386

Button A: X+12, Y+51
Button B: X+37, Y+17
Prize: X=16775, Y=15119

Button A: X+22, Y+60
Button B: X+59, Y+28
Prize: X=7431, Y=2100

Button A: X+71, Y+27
Button B: X+20, Y+59
Prize: X=5683, Y=9988

Button A: X+53, Y+17
Button B: X+15, Y+48
Prize: X=9147, Y=2208

Button A: X+21, Y+87
Button B: X+90, Y+13
Prize: X=1317, Y=778

Button A: X+38, Y+94
Button B: X+40, Y+19
Prize: X=3086, Y=5795

Button A: X+34, Y+62
Button B: X+45, Y+12
Prize: X=9746, Y=12542

Button A: X+29, Y+98
Button B: X+92, Y+15
Prize: X=7971, Y=8295

Button A: X+56, Y+12
Button B: X+20, Y+80
Prize: X=8056, Y=19772

Button A: X+25, Y+52
Button B: X+38, Y+15
Prize: X=8663, Y=3661

Button A: X+68, Y+60
Button B: X+11, Y+69
Prize: X=3070, Y=4962

Button A: X+29, Y+65
Button B: X+46, Y+17
Prize: X=16489, Y=15013

Button A: X+43, Y+24
Button B: X+18, Y+52
Prize: X=13743, Y=7816

Button A: X+87, Y+34
Button B: X+34, Y+47
Prize: X=3336, Y=1506

Button A: X+17, Y+61
Button B: X+91, Y+78
Prize: X=7436, Y=8788

Button A: X+15, Y+37
Button B: X+50, Y+13
Prize: X=19305, Y=19061

Button A: X+84, Y+23
Button B: X+13, Y+75
Prize: X=18181, Y=1490

Button A: X+73, Y+12
Button B: X+32, Y+58
Prize: X=5695, Y=1780

Button A: X+18, Y+60
Button B: X+80, Y+37
Prize: X=3460, Y=15864

Button A: X+63, Y+26
Button B: X+28, Y+61
Prize: X=6360, Y=16905

Button A: X+11, Y+55
Button B: X+37, Y+16
Prize: X=3532, Y=15555

Button A: X+67, Y+16
Button B: X+16, Y+47
Prize: X=11623, Y=18093

Button A: X+15, Y+69
Button B: X+50, Y+12
Prize: X=4300, Y=14744

Button A: X+70, Y+46
Button B: X+22, Y+48
Prize: X=8274, Y=3270

Button A: X+70, Y+20
Button B: X+86, Y+93
Prize: X=5636, Y=2363

Button A: X+82, Y+34
Button B: X+12, Y+58
Prize: X=11660, Y=11164

Button A: X+34, Y+16
Button B: X+20, Y+46
Prize: X=600, Y=10964

Button A: X+90, Y+37
Button B: X+26, Y+98
Prize: X=3144, Y=3388

Button A: X+56, Y+22
Button B: X+28, Y+65
Prize: X=2244, Y=403

Button A: X+82, Y+25
Button B: X+14, Y+15
Prize: X=1684, Y=1490

Button A: X+15, Y+42
Button B: X+55, Y+25
Prize: X=15825, Y=17469

Button A: X+11, Y+39
Button B: X+68, Y+14
Prize: X=2069, Y=12193

Button A: X+31, Y+11
Button B: X+46, Y+80
Prize: X=576, Y=6208

Button A: X+35, Y+84
Button B: X+83, Y+45
Prize: X=5260, Y=8769

Button A: X+30, Y+14
Button B: X+31, Y+47
Prize: X=16142, Y=11518

Button A: X+67, Y+11
Button B: X+22, Y+63
Prize: X=6432, Y=4420

Button A: X+68, Y+24
Button B: X+20, Y+70
Prize: X=13400, Y=14060

Button A: X+16, Y+42
Button B: X+66, Y+36
Prize: X=5332, Y=7436

Button A: X+13, Y+78
Button B: X+74, Y+61
Prize: X=1962, Y=7942

Button A: X+75, Y+20
Button B: X+11, Y+55
Prize: X=4144, Y=2615

Button A: X+11, Y+57
Button B: X+53, Y+18
Prize: X=2307, Y=11513

Button A: X+18, Y+73
Button B: X+56, Y+23
Prize: X=5408, Y=4583

Button A: X+24, Y+55
Button B: X+48, Y+13
Prize: X=4448, Y=5003

Button A: X+58, Y+63
Button B: X+93, Y+18
Prize: X=5128, Y=5238

Button A: X+51, Y+57
Button B: X+87, Y+19
Prize: X=4254, Y=1938

Button A: X+19, Y+52
Button B: X+85, Y+55
Prize: X=1883, Y=2489

Button A: X+64, Y+13
Button B: X+13, Y+59
Prize: X=1149, Y=2995

Button A: X+23, Y+60
Button B: X+86, Y+60
Prize: X=3860, Y=3660

Button A: X+33, Y+70
Button B: X+50, Y+11
Prize: X=6707, Y=5720

Button A: X+98, Y+45
Button B: X+20, Y+30
Prize: X=5890, Y=4245

Button A: X+23, Y+64
Button B: X+53, Y+14
Prize: X=2194, Y=9622

Button A: X+79, Y+38
Button B: X+12, Y+52
Prize: X=13581, Y=5494

Button A: X+23, Y+50
Button B: X+55, Y+18
Prize: X=13049, Y=7630

Button A: X+11, Y+54
Button B: X+73, Y+34
Prize: X=4237, Y=3370

Button A: X+43, Y+19
Button B: X+21, Y+37
Prize: X=7035, Y=331

Button A: X+22, Y+43
Button B: X+37, Y+16
Prize: X=4575, Y=13038

Button A: X+50, Y+18
Button B: X+16, Y+69
Prize: X=19696, Y=11834

Button A: X+27, Y+62
Button B: X+94, Y+63
Prize: X=9374, Y=6546

Button A: X+33, Y+18
Button B: X+12, Y+49
Prize: X=10055, Y=13962

Button A: X+49, Y+15
Button B: X+17, Y+38
Prize: X=5038, Y=1903

Button A: X+46, Y+65
Button B: X+33, Y+15
Prize: X=1322, Y=5090

Button A: X+15, Y+30
Button B: X+53, Y+25
Prize: X=12907, Y=7140

Button A: X+65, Y+36
Button B: X+20, Y+48
Prize: X=1550, Y=1080

Button A: X+37, Y+14
Button B: X+20, Y+45
Prize: X=14779, Y=19088

Button A: X+27, Y+58
Button B: X+41, Y+23
Prize: X=12475, Y=5911

Button A: X+36, Y+15
Button B: X+17, Y+49
Prize: X=13187, Y=9797

Button A: X+15, Y+42
Button B: X+71, Y+45
Prize: X=7631, Y=16316

Button A: X+96, Y+48
Button B: X+34, Y+55
Prize: X=4028, Y=2546

Button A: X+44, Y+26
Button B: X+49, Y+89
Prize: X=3683, Y=3077

Button A: X+12, Y+83
Button B: X+73, Y+11
Prize: X=8495, Y=15633

Button A: X+51, Y+13
Button B: X+16, Y+77
Prize: X=10044, Y=16925

Button A: X+67, Y+22
Button B: X+19, Y+61
Prize: X=7050, Y=3159

Button A: X+88, Y+21
Button B: X+42, Y+97
Prize: X=7254, Y=1992

Button A: X+78, Y+19
Button B: X+34, Y+41
Prize: X=4778, Y=3585

Button A: X+62, Y+19
Button B: X+31, Y+73
Prize: X=3773, Y=18083

Button A: X+28, Y+66
Button B: X+76, Y+55
Prize: X=9356, Y=10384

Button A: X+33, Y+83
Button B: X+53, Y+11
Prize: X=8763, Y=9909

Button A: X+46, Y+12
Button B: X+35, Y+64
Prize: X=6644, Y=11608

Button A: X+47, Y+17
Button B: X+41, Y+77
Prize: X=15413, Y=4925

Button A: X+38, Y+13
Button B: X+40, Y+75
Prize: X=6848, Y=13928

Button A: X+18, Y+45
Button B: X+25, Y+15
Prize: X=17133, Y=10400

Button A: X+16, Y+76
Button B: X+61, Y+14
Prize: X=16947, Y=666

Button A: X+13, Y+37
Button B: X+54, Y+34
Prize: X=19571, Y=10963

Button A: X+16, Y+37
Button B: X+66, Y+24
Prize: X=9800, Y=10640

Button A: X+45, Y+21
Button B: X+48, Y+83
Prize: X=6828, Y=8398

Button A: X+12, Y+22
Button B: X+50, Y+21
Prize: X=8444, Y=1198

Button A: X+62, Y+51
Button B: X+11, Y+70
Prize: X=5407, Y=6581

Button A: X+28, Y+11
Button B: X+21, Y+68
Prize: X=19219, Y=16319

Button A: X+99, Y+35
Button B: X+43, Y+79
Prize: X=2211, Y=2887

Button A: X+56, Y+26
Button B: X+33, Y+60
Prize: X=3067, Y=3586

Button A: X+24, Y+44
Button B: X+31, Y+14
Prize: X=16060, Y=1468

Button A: X+11, Y+51
Button B: X+71, Y+34
Prize: X=8684, Y=15629

Button A: X+11, Y+44
Button B: X+81, Y+43
Prize: X=18983, Y=18130

Button A: X+24, Y+40
Button B: X+43, Y+14
Prize: X=16053, Y=8370

Button A: X+36, Y+19
Button B: X+37, Y+88
Prize: X=5509, Y=7906

Button A: X+46, Y+79
Button B: X+80, Y+21
Prize: X=8446, Y=7638

Button A: X+46, Y+24
Button B: X+16, Y+53
Prize: X=444, Y=5428

Button A: X+38, Y+69
Button B: X+31, Y+11
Prize: X=18308, Y=4776

Button A: X+47, Y+19
Button B: X+21, Y+40
Prize: X=5815, Y=19032

Button A: X+12, Y+49
Button B: X+85, Y+47
Prize: X=6494, Y=18488

Button A: X+56, Y+18
Button B: X+28, Y+61
Prize: X=10336, Y=13904

Button A: X+53, Y+12
Button B: X+27, Y+80
Prize: X=913, Y=11260

Button A: X+16, Y+78
Button B: X+64, Y+15
Prize: X=2880, Y=15044

Button A: X+43, Y+96
Button B: X+60, Y+21
Prize: X=5225, Y=9519

Button A: X+11, Y+27
Button B: X+58, Y+41
Prize: X=6653, Y=14661

Button A: X+12, Y+54
Button B: X+65, Y+61
Prize: X=4044, Y=7086

Button A: X+25, Y+70
Button B: X+56, Y+15
Prize: X=14097, Y=9250

Button A: X+11, Y+73
Button B: X+52, Y+14
Prize: X=3135, Y=8989

Button A: X+92, Y+39
Button B: X+25, Y+44
Prize: X=5781, Y=4889

Button A: X+24, Y+40
Button B: X+50, Y+22
Prize: X=3246, Y=3018

Button A: X+60, Y+42
Button B: X+16, Y+36
Prize: X=12568, Y=16796

Button A: X+75, Y+40
Button B: X+14, Y+49
Prize: X=19537, Y=16492

Button A: X+72, Y+31
Button B: X+43, Y+84
Prize: X=3886, Y=2328

Button A: X+72, Y+14
Button B: X+12, Y+77
Prize: X=3284, Y=19387

Button A: X+69, Y+37
Button B: X+38, Y+89
Prize: X=6982, Y=7381

Button A: X+60, Y+15
Button B: X+16, Y+57
Prize: X=10492, Y=1814

Button A: X+52, Y+72
Button B: X+68, Y+28
Prize: X=6104, Y=3424

Button A: X+19, Y+12
Button B: X+20, Y+58
Prize: X=1189, Y=2974

Button A: X+65, Y+33
Button B: X+24, Y+47
Prize: X=16767, Y=13934

Button A: X+76, Y+13
Button B: X+44, Y+97
Prize: X=1812, Y=3531

Button A: X+41, Y+22
Button B: X+38, Y+76
Prize: X=5043, Y=4986

Button A: X+41, Y+19
Button B: X+21, Y+73
Prize: X=4361, Y=6513

Button A: X+25, Y+97
Button B: X+37, Y+31
Prize: X=2121, Y=4515

Button A: X+21, Y+48
Button B: X+56, Y+22
Prize: X=12653, Y=1688

Button A: X+63, Y+34
Button B: X+34, Y+64
Prize: X=16084, Y=18336

Button A: X+53, Y+17
Button B: X+31, Y+62
Prize: X=11230, Y=8291

Button A: X+78, Y+15
Button B: X+94, Y+87
Prize: X=7660, Y=6918

Button A: X+82, Y+25
Button B: X+12, Y+52
Prize: X=8056, Y=3858

Button A: X+78, Y+48
Button B: X+27, Y+80
Prize: X=4581, Y=6432

Button A: X+41, Y+12
Button B: X+26, Y+65
Prize: X=7348, Y=4215

Button A: X+66, Y+87
Button B: X+87, Y+32
Prize: X=6324, Y=3706

Button A: X+53, Y+14
Button B: X+18, Y+53
Prize: X=8546, Y=9263

Button A: X+15, Y+48
Button B: X+52, Y+20
Prize: X=1621, Y=3284

Button A: X+48, Y+90
Button B: X+66, Y+11
Prize: X=2250, Y=3655

Button A: X+13, Y+47
Button B: X+59, Y+18
Prize: X=9432, Y=2691

Button A: X+38, Y+11
Button B: X+16, Y+67
Prize: X=13954, Y=19138

Button A: X+13, Y+38
Button B: X+44, Y+16
Prize: X=9077, Y=11150

Button A: X+84, Y+31
Button B: X+48, Y+98
Prize: X=6756, Y=10281

Button A: X+15, Y+50
Button B: X+73, Y+23
Prize: X=3235, Y=18700

Button A: X+23, Y+41
Button B: X+67, Y+20
Prize: X=4963, Y=2881

Button A: X+18, Y+65
Button B: X+56, Y+15
Prize: X=4992, Y=5655

Button A: X+50, Y+17
Button B: X+28, Y+51
Prize: X=14692, Y=5242

Button A: X+65, Y+15
Button B: X+14, Y+53
Prize: X=9264, Y=10918

Button A: X+71, Y+34
Button B: X+30, Y+81
Prize: X=2598, Y=4176

Button A: X+62, Y+22
Button B: X+34, Y+76
Prize: X=17948, Y=3296

Button A: X+80, Y+23
Button B: X+53, Y+72
Prize: X=5276, Y=2198

Button A: X+49, Y+25
Button B: X+36, Y+56
Prize: X=18951, Y=2903

Button A: X+51, Y+12
Button B: X+14, Y+44
Prize: X=9882, Y=6180

Button A: X+54, Y+74
Button B: X+25, Y+11
Prize: X=4031, Y=4989

Button A: X+27, Y+56
Button B: X+35, Y+21
Prize: X=10038, Y=7662

Button A: X+33, Y+80
Button B: X+67, Y+45
Prize: X=6928, Y=9280

Button A: X+17, Y+14
Button B: X+11, Y+85
Prize: X=1338, Y=2241

Button A: X+32, Y+44
Button B: X+37, Y+14
Prize: X=4167, Y=16174

Button A: X+34, Y+56
Button B: X+82, Y+26
Prize: X=9362, Y=4732

Button A: X+55, Y+87
Button B: X+81, Y+42
Prize: X=7654, Y=7887

Button A: X+34, Y+73
Button B: X+43, Y+20
Prize: X=1346, Y=2456

Button A: X+14, Y+22
Button B: X+25, Y+13
Prize: X=4534, Y=10958

Button A: X+24, Y+71
Button B: X+52, Y+16
Prize: X=12916, Y=3562

Button A: X+23, Y+57
Button B: X+33, Y+18
Prize: X=9501, Y=12137

Button A: X+17, Y+99
Button B: X+76, Y+51
Prize: X=7738, Y=7470

Button A: X+28, Y+85
Button B: X+69, Y+13
Prize: X=15991, Y=13874

Button A: X+26, Y+68
Button B: X+68, Y+20
Prize: X=17416, Y=17224

Button A: X+86, Y+17
Button B: X+55, Y+51
Prize: X=3871, Y=3213

Button A: X+37, Y+11
Button B: X+26, Y+67
Prize: X=10077, Y=3613

Button A: X+18, Y+30
Button B: X+97, Y+50
Prize: X=1859, Y=1870

Button A: X+59, Y+32
Button B: X+34, Y+58
Prize: X=14268, Y=8286

Button A: X+27, Y+54
Button B: X+56, Y+33
Prize: X=4651, Y=6143

Button A: X+88, Y+38
Button B: X+18, Y+57
Prize: X=9788, Y=8854

Button A: X+62, Y+18
Button B: X+12, Y+59
Prize: X=16650, Y=1065

Button A: X+48, Y+66
Button B: X+35, Y+13
Prize: X=11648, Y=14762

Button A: X+82, Y+57
Button B: X+40, Y+92
Prize: X=3748, Y=4146

Button A: X+78, Y+13
Button B: X+58, Y+52
Prize: X=12570, Y=6032

Button A: X+28, Y+56
Button B: X+60, Y+35
Prize: X=12124, Y=2363

Button A: X+79, Y+30
Button B: X+15, Y+51
Prize: X=1666, Y=7601

Button A: X+60, Y+12
Button B: X+15, Y+61
Prize: X=17825, Y=5203

Button A: X+33, Y+17
Button B: X+14, Y+23
Prize: X=6805, Y=19369

Button A: X+13, Y+41
Button B: X+75, Y+30
Prize: X=8216, Y=3962

Button A: X+12, Y+42
Button B: X+74, Y+24
Prize: X=17410, Y=9800

Button A: X+95, Y+47
Button B: X+28, Y+97
Prize: X=1169, Y=2075

Button A: X+44, Y+62
Button B: X+98, Y+22
Prize: X=12870, Y=6642

Button A: X+62, Y+20
Button B: X+21, Y+51
Prize: X=1885, Y=11497

Button A: X+38, Y+68
Button B: X+49, Y+22
Prize: X=14751, Y=2442

Button A: X+47, Y+14
Button B: X+16, Y+64
Prize: X=1315, Y=2902

Button A: X+36, Y+69
Button B: X+84, Y+14
Prize: X=7416, Y=6276

Button A: X+79, Y+19
Button B: X+35, Y+82
Prize: X=4501, Y=8220

Button A: X+79, Y+12
Button B: X+69, Y+90
Prize: X=12481, Y=8814

Button A: X+93, Y+59
Button B: X+30, Y+94
Prize: X=8118, Y=7774

Button A: X+47, Y+93
Button B: X+91, Y+29
Prize: X=10288, Y=8272

Button A: X+60, Y+20
Button B: X+48, Y+66
Prize: X=3588, Y=4496

Button A: X+15, Y+66
Button B: X+56, Y+19
Prize: X=3688, Y=1515

Button A: X+65, Y+15
Button B: X+26, Y+70
Prize: X=16500, Y=12300

Button A: X+42, Y+38
Button B: X+87, Y+12
Prize: X=7665, Y=3666

Button A: X+85, Y+42
Button B: X+53, Y+97
Prize: X=4910, Y=7737

Button A: X+96, Y+52
Button B: X+35, Y+65
Prize: X=4301, Y=5967

Button A: X+94, Y+78
Button B: X+16, Y+74
Prize: X=3226, Y=5288

Button A: X+31, Y+76
Button B: X+50, Y+15
Prize: X=7652, Y=15977

Button A: X+11, Y+52
Button B: X+78, Y+26
Prize: X=1476, Y=2522

Button A: X+24, Y+13
Button B: X+24, Y+42
Prize: X=15416, Y=11932

Button A: X+27, Y+59
Button B: X+58, Y+18
Prize: X=10884, Y=2044

Button A: X+65, Y+14
Button B: X+50, Y+48
Prize: X=9195, Y=5294

Button A: X+56, Y+23
Button B: X+18, Y+64
Prize: X=6606, Y=10568

Button A: X+20, Y+59
Button B: X+60, Y+24
Prize: X=11300, Y=9950

Button A: X+79, Y+24
Button B: X+14, Y+64
Prize: X=10080, Y=15680

Button A: X+80, Y+29
Button B: X+34, Y+42
Prize: X=5076, Y=2849

Button A: X+39, Y+15
Button B: X+34, Y+60
Prize: X=11578, Y=17000

Button A: X+82, Y+38
Button B: X+18, Y+34
Prize: X=4504, Y=2780

Button A: X+30, Y+19
Button B: X+19, Y+43
Prize: X=18763, Y=12159

Button A: X+32, Y+33
Button B: X+75, Y+18
Prize: X=7155, Y=2097

Button A: X+42, Y+14
Button B: X+30, Y+67
Prize: X=8144, Y=12035

Button A: X+23, Y+43
Button B: X+47, Y+21
Prize: X=15418, Y=7394

Button A: X+38, Y+99
Button B: X+77, Y+49
Prize: X=7550, Y=9967

Button A: X+13, Y+61
Button B: X+28, Y+11
Prize: X=3003, Y=4701

Button A: X+50, Y+13
Button B: X+65, Y+92
Prize: X=8430, Y=8350

Button A: X+48, Y+71
Button B: X+41, Y+13
Prize: X=2935, Y=15156

Button A: X+61, Y+40
Button B: X+32, Y+93
Prize: X=5820, Y=10874

Button A: X+73, Y+12
Button B: X+11, Y+45
Prize: X=2352, Y=12983

Button A: X+41, Y+17
Button B: X+19, Y+55
Prize: X=6540, Y=7920

Button A: X+70, Y+28
Button B: X+24, Y+64
Prize: X=6710, Y=11820

Button A: X+16, Y+71
Button B: X+75, Y+21
Prize: X=4864, Y=18423

Button A: X+93, Y+16
Button B: X+31, Y+43
Prize: X=6851, Y=4305

Button A: X+65, Y+28
Button B: X+15, Y+42
Prize: X=4895, Y=4312

Button A: X+14, Y+21
Button B: X+37, Y+15
Prize: X=8114, Y=11906

Button A: X+15, Y+69
Button B: X+40, Y+13
Prize: X=5765, Y=17537

Button A: X+56, Y+15
Button B: X+23, Y+64
Prize: X=2790, Y=12835

Button A: X+55, Y+17
Button B: X+12, Y+60
Prize: X=4915, Y=1077

Button A: X+77, Y+12
Button B: X+15, Y+61
Prize: X=833, Y=10411

Button A: X+84, Y+52
Button B: X+29, Y+82
Prize: X=3746, Y=3728

Button A: X+22, Y+68
Button B: X+80, Y+70
Prize: X=6040, Y=6260

Button A: X+39, Y+15
Button B: X+38, Y+59
Prize: X=15452, Y=3962

Button A: X+34, Y+15
Button B: X+26, Y+62
Prize: X=9220, Y=2426

Button A: X+48, Y+44
Button B: X+14, Y+58
Prize: X=5134, Y=7642

Button A: X+99, Y+87
Button B: X+86, Y+12
Prize: X=8829, Y=2037

Button A: X+95, Y+13
Button B: X+30, Y+84
Prize: X=4540, Y=3098

Button A: X+13, Y+95
Button B: X+95, Y+77
Prize: X=9667, Y=13241

Button A: X+62, Y+19
Button B: X+25, Y+69
Prize: X=3712, Y=6724

Button A: X+23, Y+54
Button B: X+48, Y+28
Prize: X=8619, Y=18970

Button A: X+27, Y+12
Button B: X+23, Y+41
Prize: X=1893, Y=1857

Button A: X+25, Y+59
Button B: X+53, Y+17
Prize: X=12619, Y=11763

Button A: X+19, Y+25
Button B: X+50, Y+17
Prize: X=2982, Y=2460

Button A: X+40, Y+79
Button B: X+61, Y+28
Prize: X=6621, Y=5586

Button A: X+12, Y+56
Button B: X+59, Y+19
Prize: X=11964, Y=6068

Button A: X+37, Y+95
Button B: X+56, Y+39
Prize: X=6635, Y=10644

Button A: X+67, Y+27
Button B: X+17, Y+48
Prize: X=5655, Y=9518

Button A: X+66, Y+12
Button B: X+18, Y+52
Prize: X=19562, Y=1988

Button A: X+21, Y+57
Button B: X+66, Y+32
Prize: X=11801, Y=5977

Button A: X+14, Y+55
Button B: X+82, Y+39
Prize: X=412, Y=8162

Button A: X+53, Y+27
Button B: X+11, Y+28
Prize: X=16579, Y=8710

Button A: X+20, Y+70
Button B: X+70, Y+26
Prize: X=8530, Y=9390

Button A: X+35, Y+11
Button B: X+51, Y+83
Prize: X=5094, Y=11694

Button A: X+28, Y+44
Button B: X+72, Y+15
Prize: X=3648, Y=1905

Button A: X+64, Y+14
Button B: X+42, Y+96
Prize: X=7374, Y=8124

Button A: X+25, Y+87
Button B: X+79, Y+36
Prize: X=1809, Y=1278

Button A: X+64, Y+87
Button B: X+70, Y+18
Prize: X=9860, Y=9237

Button A: X+93, Y+56
Button B: X+14, Y+92
Prize: X=9180, Y=7032

Button A: X+70, Y+12
Button B: X+12, Y+37
Prize: X=10884, Y=1351

Button A: X+92, Y+70
Button B: X+13, Y+85
Prize: X=6266, Y=6420

Button A: X+13, Y+91
Button B: X+94, Y+38
Prize: X=9417, Y=5779

Button A: X+54, Y+22
Button B: X+15, Y+27
Prize: X=7742, Y=16598

Button A: X+23, Y+55
Button B: X+52, Y+18
Prize: X=9181, Y=7813

Button A: X+17, Y+35
Button B: X+46, Y+23
Prize: X=3863, Y=16241

Button A: X+29, Y+11
Button B: X+18, Y+32
Prize: X=17920, Y=6300

Button A: X+34, Y+80
Button B: X+99, Y+66
Prize: X=3554, Y=3688

Button A: X+19, Y+40
Button B: X+79, Y+33
Prize: X=8099, Y=5852

Button A: X+96, Y+33
Button B: X+31, Y+49
Prize: X=4001, Y=5018

Button A: X+65, Y+46
Button B: X+20, Y+87
Prize: X=3770, Y=9297

Button A: X+13, Y+38
Button B: X+44, Y+31
Prize: X=8799, Y=18736

Button A: X+73, Y+45
Button B: X+17, Y+47
Prize: X=12662, Y=350

Button A: X+21, Y+51
Button B: X+59, Y+25
Prize: X=17122, Y=3246

Button A: X+49, Y+28
Button B: X+20, Y+47
Prize: X=18656, Y=16322

Button A: X+60, Y+14
Button B: X+28, Y+62
Prize: X=13800, Y=15636

Button A: X+52, Y+30
Button B: X+15, Y+36
Prize: X=9941, Y=4094

Button A: X+65, Y+35
Button B: X+13, Y+46
Prize: X=14589, Y=513

Button A: X+93, Y+59
Button B: X+23, Y+84
Prize: X=3917, Y=5886

Button A: X+22, Y+54
Button B: X+86, Y+16
Prize: X=4048, Y=5644

Button A: X+62, Y+23
Button B: X+23, Y+37
Prize: X=6447, Y=3388

Button A: X+77, Y+36
Button B: X+24, Y+79
Prize: X=4298, Y=6754

Button A: X+54, Y+29
Button B: X+11, Y+25
Prize: X=5275, Y=10524

Button A: X+67, Y+74
Button B: X+95, Y+30
Prize: X=10602, Y=7364

Button A: X+18, Y+42
Button B: X+98, Y+65
Prize: X=4582, Y=4963

Button A: X+70, Y+25
Button B: X+16, Y+64
Prize: X=4324, Y=11566

Button A: X+48, Y+14
Button B: X+35, Y+75
Prize: X=1662, Y=6676

Button A: X+20, Y+45
Button B: X+62, Y+25
Prize: X=9014, Y=17975

Button A: X+11, Y+53
Button B: X+74, Y+31
Prize: X=2157, Y=4533

Button A: X+88, Y+30
Button B: X+42, Y+72
Prize: X=7834, Y=6420

Button A: X+78, Y+39
Button B: X+34, Y+62
Prize: X=8208, Y=8154

Button A: X+12, Y+38
Button B: X+71, Y+47
Prize: X=15341, Y=14727

Button A: X+73, Y+51
Button B: X+19, Y+94
Prize: X=6062, Y=10451

Button A: X+21, Y+84
Button B: X+44, Y+21
Prize: X=4675, Y=9555

Button A: X+58, Y+75
Button B: X+81, Y+31
Prize: X=5008, Y=5296

Button A: X+54, Y+23
Button B: X+15, Y+42
Prize: X=4154, Y=3125

Button A: X+65, Y+20
Button B: X+15, Y+39
Prize: X=8845, Y=1030

Button A: X+20, Y+75
Button B: X+93, Y+70
Prize: X=9687, Y=8730

Button A: X+51, Y+24
Button B: X+29, Y+59
Prize: X=13536, Y=12777

Button A: X+53, Y+11
Button B: X+39, Y+78
Prize: X=6231, Y=13482

Button A: X+24, Y+94
Button B: X+44, Y+30
Prize: X=1912, Y=2934

Button A: X+24, Y+92
Button B: X+89, Y+50
Prize: X=4351, Y=9982

Button A: X+35, Y+88
Button B: X+85, Y+28
Prize: X=4775, Y=6620

Button A: X+12, Y+19
Button B: X+44, Y+15
Prize: X=16064, Y=11340

Button A: X+36, Y+78
Button B: X+75, Y+27
Prize: X=4236, Y=4842

Button A: X+24, Y+74
Button B: X+55, Y+16
Prize: X=19430, Y=3834

Button A: X+21, Y+59
Button B: X+94, Y+17
Prize: X=10632, Y=6891

Button A: X+33, Y+13
Button B: X+43, Y+98
Prize: X=3645, Y=5570

Button A: X+24, Y+92
Button B: X+25, Y+19
Prize: X=3891, Y=9153

Button A: X+61, Y+87
Button B: X+94, Y+38
Prize: X=9321, Y=9067\
`;

interface Machine {
    a: Point;
    b: Point;
    prize: Point;
}

function parsePoint(match: RegExpExecArray | null): Point {
    if (!match?.groups) {
        throw new Error('Invalid match input');
    }

    return { x: parseInt(match.groups['x']), y: parseInt(match.groups['y']) };
}

function parseMachine(input: string): Machine {
    const buttonMatcher = /Button \w: X\+(?<x>\d+), Y\+(?<y>\d+)/;
    const prizeMatcher = /Prize: X=(?<x>\d+), Y=(?<y>\d+)/;
    const [row1, row2, row3] = input.split('\n');
    
    return {
        a: parsePoint(buttonMatcher.exec(row1)),
        b: parsePoint(buttonMatcher.exec(row2)),
        prize: parsePoint(prizeMatcher.exec(row3)),
    };
}

function countMinimumTokens(machine: Machine) {
    let aCount = 0;
    let bCount = Math.min(100, Math.floor(machine.prize.x / machine.b.x), Math.floor(machine.prize.y / machine.b.y));
    let position = { x: machine.b.x * bCount, y: machine.b.y * bCount };
    
    while (!areEqual(position, machine.prize) && bCount >= 0 && aCount <= 100) {
        while (aCount <= 100 && (position.x < machine.prize.x || position.y < machine.prize.y)) {
            position = { x: position.x + machine.a.x, y: position.y + machine.a.y };
            aCount++;
        }
        while (bCount >= 0 && (position.x > machine.prize.x || position.y > machine.prize.y)) {
            position = { x: position.x - machine.b.x, y: position.y - machine.b.y };
            bCount--;
        }
    }

    return areEqual(position, machine.prize) ? 3 * aCount + bCount : null;
}

const MACHINES = INPUT.split('\n\n').map(parseMachine);

console.log('Minimum tokens required:', MACHINES.reduce((sum, machine) => sum + (countMinimumTokens(machine) ?? 0), 0));
